## Ex1: Geneerate values (e.g 1000) of 3 random
## variables following standard normal distribution
## so that the variables are correlated as
## follows:
## Sigma = [
##           1  0.8 0.3
##          0.8  1  0.5
...
## Ex2:
set.seed(1000)
# Generates n random variables from f, where mu has the means of fi,
# w has the weights for f and var has the variances of fi
rwnorm <- function(mu, w, var, n) {
  wvec <- vector(length=n)
...
# Adding f1, f2 and f3 to the plot as colored lines
...
p
#Reject the null hypothesis, the mean is not 0
...
## Ex5: Test the hypothesis H0: muy1=muy2
## against H1: muy1 < muy2 using permutation
## test ( random test version). Calculate the
## significance level for R-data sleep, where
## the response variable is "extra" and the group 
## is "group"
...
p <- (sum((v1-v2)<d))/n
p
# rejecr the null hypothesis at the 5% level
